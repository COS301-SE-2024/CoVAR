...
const OrganizationEvaluation: React.FC = () => {
  ...
  const handleToggle = async (upload_id: number, fileName: string) => {
    ...
    try {
      ...
      // Update the state only after successful API calls
    ...
    <Container maxWidth={false} sx={{ ...mainContentStyles, paddingTop: 8, width: '100vw' }}>
      ...
      <Grid container spacing={2}>
        <Grid item xs={6}>
          <Paper sx={{ padding: 4, textAlign: 'center', overflowY: 'auto', height: '40vh' }}>
            ...
            <FileUpload onFileSubmit={handleFileSubmit} organization={organizationName ?? undefined} />
          </Paper>
          <Box mt={2} /> {/* Gap between upload files and uploaded files */}
          <Paper sx={{ padding: 4, textAlign: 'center', overflowY: 'auto', height: '40vh' }}>
      ...
      <Box display="flex" justifyContent="center" mt={2}>
        <Button
          ...
          onClick={() => router.push(`/evaluate/conflicts/${organizationName}?type=org`)}
          sx={{
            ...buttonStyles,
            marginTop: '3vh',
            width: '150px',
